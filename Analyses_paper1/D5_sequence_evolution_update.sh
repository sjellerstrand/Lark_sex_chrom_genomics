...
#SBATCH -n 1
#SBATCH -t 5:00:00
#SBATCH -J sequence_evolution_update
...
sequence_evolution() {
  ...
  if [ $REGION1 == "autosomal" ] && [ $REGION2 == "autosomal" ]; then
      BRANCHES=$(echo BranchA BranchSkyA BranchRasoA BranchO Node1);
      SUFFIX="A_A";
      GROUPS1=$(echo A);
      GROUPS2=$(echo A);
    ...
    if [ $STRATA1 == "Z" ] && [ $STRATA2 == "Z" ]; then
      BRANCHES=$(echo BranchAZ BranchSkyZ BranchRasoZ BranchO Node1);
    else
      BRANCHES=$(echo BranchAZ BranchSkyZ BranchRasoZ BranchO Node1);
    fi;
    ...
    GROUPS2=$(echo Z W);
    if [ $STRATA1 == "Z" ] && [ $STRATA2 == "Z" ]; then
      BRANCHES=$(echo BranchZ BranchSkyZ BranchRasoZ BranchW BranchSkyW BranchRasoW BranchO Node23 Node24);
    else
      BRANCHES=$(echo BranchA BranchZ BranchSkyZ BranchRasoZ BranchW BranchSkyW BranchRasoW BranchO Node1);
    fi;
    ...
    GROUPS2=$(echo Z);
    if [ $STRATA1 == "Z" ] && [ $STRATA2 == "Z" ]; then
      BRANCHES=$(echo BranchZ BranchSkyZ BranchRasoZ BranchSkyW BranchO Node12 Node13);
    else
      BRANCHES=$(echo BranchA BranchZ BranchSkyZ BranchRasoZ BranchSkyW BranchO Node12 Node13)
    fi;
    ...
    GROUPS2=$(echo Z W);
    if [ $STRATA1 == "Z" ] && [ $STRATA2 == "Z" ]; then
      BRANCHES=$(echo BranchZ BranchSkyZ BranchRasoZ BranchRasoW BranchO Node11 Node12);
    else
      BRANCHES=$(echo BranchA BranchZ BranchSkyZ BranchRasoZ BranchRasoW BranchO Node11 Node12)
    fi;
  ...
  for BRANCH in $BRANCHES; do
    ...
    if [ ${#BRANCH} -gt 8 ]; then
      if [ $(echo $BRANCH | grep Sky | wc -l) -gt 0 ]; then
        ...
        if [ $(echo $BRANCH | rev | grep ^A | wc -l) -gt 0 ]; then
          GROUP="A"
          INGROUPOMEGA=$(cat $OUTDIR/shared_genes/$REGION1\_$REGION2/$GENE/$PROJECT\_$GENE\_MG94_$GROUP\_ingroup.json | \
          grep "non-synonymous/synonymous rate ratio" | tail -n1 | cut -d':' -f2 | cut -d',' -f1);
          pN=$(cat $OUTDIR/shared_genes/$REGION1\_$REGION2/$GENE/$PROJECT\_$GENE\_MG94_$GROUP\_ingroup.json | \
          grep "\"dN\":" | grep -v "{" | cut -d':' -f2 | awk -F',' 'BEGIN{SUM=0} {SUM+=$1} END {print SUM}');
          pS=$(cat $OUTDIR/shared_genes/$REGION1\_$REGION2/$GENE/$PROJECT\_$GENE\_MG94_$GROUP\_ingroup.json | \
          grep "\"dS\":" | grep -v "{" | cut -d':' -f2 | awk -F',' 'BEGIN{SUM=0} {SUM+=$1} END {print SUM}');
    ...
    if [ $(cat $OUTDIR/shared_genes/$REGION1\_$REGION2/$GENE/$PROJECT1\_$PROJECT2\_$GENE\_MG94.json | grep $BRANCH | wc -l) == 0 ]; then
      ...
      echo -e "${GENE}\t${BRANCH}\t0\t0\t0\tNA\t0\t0\tNA\t0\t0\t$INGROUPOMEGA";
      ...
      # Report data
      echo -e "${GENE}\t${BRANCH}\t${LENGTH}\t${dN}\t${dS}\t${dNdS}\t${BETA}\t${ALPHA}\t${OMEGA}\t${pN}\t${pS}\t${INGROUPOMEGA}";
