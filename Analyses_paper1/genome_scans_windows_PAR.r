...
### Import data
regions <- read.delim(paste(OUTDIR, "/", PROJECT, "_callable_", DATA, ".bed", sep=""), sep="\t", head=F)
...
### Write file
write.table(data, file=paste(OUTDIR, "/window_", WINDOW, "_step_", STEP, "/", PROJECT, "_window_", WINDOW, "_step_", STEP, "_", DATA, ".txt", sep=""), quote=FALSE, sep="\t", row.names = F, col.names = T)
