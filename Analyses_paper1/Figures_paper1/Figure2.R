...
# Get odds_ratios
coef <-  summary(FinalModel)$coefficients$cond
...
png("Figures/Supplementary/degeneration_all_regions.png", width=4500, height=3000, res=300)
plot_regions1
dev.off()
...
plot_OR2 <- ggplot(odds_ratios_df, aes(x = contVar, y = logOdds_Ratio)) +
...
png("Figures/Supplementary/plot_OR2.png", width=3000, height=2000, res=300)
plot_OR2
dev.off()

taxa_deg <- tibble(
  Species = c(
    "Silene latifolia (XY)", "Silene latifolia (XY)",
    "Drosophila miranda (XY)",
    "Homo sapiens (XY)", "Homo sapiens (XY)", "Homo sapiens (XY)",
    "Tinamus guttatus (ZW)", "Tinamus guttatus (ZW)",
    "Mesitornis unicolor (ZW)", "Mesitornis unicolor (ZW)",
    "Phaethon lepturus (ZW)", "Phaethon lepturus (ZW)",
    "Nipponia nippon (ZW)", "Nipponia nippon (ZW)",
    "Tyto alba (ZW)", "Tyto alba (ZW)"),
  Age_Ma = c(11.00, 6.30, 1.75, 150, 100, 50, 50.00, 29.00, 69.00, 48.00, 96.00, 70.00, 69.00, 53.00, 69.00, 49.00),
  Age_MG = c(7.33, 4.20, 17.50, 15.00, 6.67, 2.50, 7.35, 4.26, 16.43, 11.43, 8.65, 6.31, 6.83, 5.25, 11.13, 7.60),
  Percent_Degen = c(51.04, 43.73, 46.55, 83.33, 71.43, 41.67, 89.50, 76.84, 91.08, 66.72, 96.00, 83.30, 85.56, 91.69, 93.84, 86.24),
  Sex_system = c("XY", "XY", "XY", "XY", "XY", "XY", "ZW", "ZW", "ZW", "ZW", "ZW", "ZW", "ZW", "ZW", "ZW", "ZW"))
taxa_deg$prop <- taxa_deg$Percent_Degen/100
taxa_deg$Sex_system <- factor(taxa_deg$Sex_system)
taxa_deg <- taxa_deg[order(taxa_deg$Sex_system, taxa_deg$Species), ]
taxa_deg$Species <- factor(taxa_deg$Species, levels = unique(taxa_deg$Species))



plot_taxa_deg <- ggplot() +
  geom_line(data=pred_seq1_age, aes(x=Strata_Age_Generations, y=LOF, color="Alauda (ZW)", linetype="ZW"), linewidth=1) +
  geom_smooth(data = taxa_deg, aes(x = Age_MG, y = prop, color = Species, linetype= Sex_system),  method = "glm", method.args = list(family = quasibinomial(link = "logit")), se = FALSE, linewidth = 1.2) +
  geom_point(data=pred_seq2_age, aes(x=Strata_Age_Generations, y=prop, color="Alauda (ZW)", shape="ZW"), size=3.5) +
  geom_point(data=taxa_deg, aes(x=Age_MG, y=prop, color=Species, shape=Sex_system), size=3.5) +
  scale_shape_manual(values = c("XY" = 17, "ZW" = 16)) +
  scale_linetype_manual(values = c("XY" = "dashed", "ZW" = "solid")) +
  scale_x_continuous() +
  scale_y_continuous(limits=c(0,1)) +
  labs(x ="Age (million generations)", y = expression(atop("Proportion of Degeneration")), title = NULL) +
...
png("Figures/Supplementary/plot_taxa_deg", width=3000, height=2000, res=300)
plot_taxa_deg
dev.off()
...
plot_curve_age <- ggplot() +
  ...
  scale_y_continuous(limits=c(0,1)) +
  geom_text(data = pred_seq2_age, aes(x=Strata_Age_Generations, y=prop, label = Strata, hjust = c(-0.5,-0.5,-0.5,-0.5,-0.5,-0.5,-0.5,-0.5,-0.5,-0.5), vjust = c(1.5,1.5,1.5,0.8,1.5,1.5,-0.5,1.5,1.5,1.5)), color = "Black", size=4.5) +
...
plot_curves_len <- ggplot() +
  ...
  annotate(geom="text", x=c(4.25, 3.9, 3.25, 2.8, 2.3) , y=c(0.16, 0.36, 0.65, 0.90, 0.97), label=c("Age = 0 MG", "Age = 5 MG", "Age = 10 MG", "Age = 15 MG", "Age = 20 MG"), color="Black", size=4.5) +
  guides(color=guide_legend(title="Data")) +
  theme_bw() +
...
plot_curves_phap <- ggplot() +
  ...
  annotate(geom="text", x=c(0.39, 0.52, 0.65, 0.78, 0.89) , y=c(0.01, 0.19, 0.55, 0.78, 0.90), label=c("Age = 0 MG", "Age = 5 MG", "Age = 10 MG", "Age = 15 MG", "Age = 20 MG"), color="Black", size=4.5) +
  guides(color=guide_legend(title="Data")) +
  theme_bw() +
  theme(legend.position= "none",
        legend.key.size = unit(1, 'line'), #change legend key size
...
plot_curves_len_phap <- ggplot() +
  ...
  scale_x_continuous(breaks=c(2, 2.5, 3, 3.5, 4, 4.5), limits=c(2, 4.5)) +
  scale_y_continuous(breaks=c(0, 0.2, 0.4, 0.6, 0.8, 1), limits = c(0, 1)) +
  labs(x =expression("log"[10]*"(Gene length)"), y = expression(atop("Probability of", "non-functionality"))) +
  annotate(geom="text", x=c(3.0, 3.3, 3.45, 3.6, 3.8, 4) , y=c(0.9, 0.8, 0.7, 0.55, 0.4,  0.25), label=c("pHaplo = 0.0", "pHaplo = 0.2", "pHaplo = 0.4", "pHaplo = 0.6", "pHaplo = 0.8", "pHaplo = 1.0"), color="Black", size=4.5) +
  guides(color=guide_legend(title="Data")) +
  theme_bw() +
  theme(legend.position= "none",
        legend.key.size = unit(1, 'line'), #change legend key size
...
fig2_plot <- plot_regions2 / plot_curve_age / (plot_curves_len + plot_curves_phap + plot_curves_len_phap) +
...
png("Figures/Figure2.png", width=5250, height=3500, res=300)
fig2_plot
dev.off()
